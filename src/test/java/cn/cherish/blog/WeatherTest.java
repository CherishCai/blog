...
public class WeatherTest {
    ...
    public static void main(String[] args) {
        ...
        String httpArg = "city=广州";
        String jsonResult = request(httpUrl, httpArg);

        if(jsonResult.indexOf("ok") > 0){

            JSONObject parse = JSONObject.parseObject(jsonResult);
            String str = parse.get("HeWeather data service 3.0").toString();
            String substring = str.substring(1, str.length()-1);

            WeatherTest weatherTest = JSON.parseObject(substring, WeatherTest.class);
            System.out.println("aqi:---"+weatherTest.aqi);
            System.out.println("basic:---"+weatherTest.basic);
            System.out.println("now:---"+weatherTest.now);
            System.out.println("suggestion:---"+weatherTest.suggestion);
            System.out.println("daily_forecast:--@@@@@@@@@@@@@@@@@@@@@@@@@@@--");

            for (DailyForecastBean df : weatherTest.daily_forecast) {
                System.out.println(df);
            }
            System.out.println("hourly_forecast:#################################");

            for (HourlyForecastBean hf : weatherTest.hourly_forecast) {
                System.out.println(hf);
            }
        }else {
            System.out.println("不好意思，查无此城！");
        }
    }
