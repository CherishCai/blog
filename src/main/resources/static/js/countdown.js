...
var balls = [];
const
colors = [ "#33B5E5", "#0099CC", "#AA66CC", "#9933CC", "#99CC00", "#669900",
...
window.onload = function() {
	...
	window.addEventListener("resize",myreset);
}
...
function update() {
	...
	if (nextSeconds != curSeconds) {
		...
		if (parseInt(nextHours / 10) != parseInt(curHours / 10)) {
			allBalls(MARGIN_LEFT + 0, MARGIN_TOP, parseInt(curHours / 10));
		}
		if (parseInt(nextHours % 10) != parseInt(curHours % 10)) {
			allBalls(MARGIN_LEFT + 15 * (RADIUS + 1), MARGIN_TOP,
					parseInt(curHours % 10));
		}

		if (parseInt(nextMinutes / 10) != parseInt(curMinutes / 10)) {
			allBalls(MARGIN_LEFT + 39 * (RADIUS + 1), MARGIN_TOP,
					parseInt(curHours / 10));
		}
		if (parseInt(nextMinutes % 10) != parseInt(curMinutes % 10)) {
			allBalls(MARGIN_LEFT + 54 * (RADIUS + 1), MARGIN_TOP,
					parseInt(curHours % 10));
		}

		if (parseInt(nextSeconds / 10) != parseInt(curSeconds / 10)) {
			allBalls(MARGIN_LEFT + 78 * (RADIUS + 1), MARGIN_TOP,
					parseInt(curHours / 10));
		}
		if (parseInt(nextSeconds % 10) != parseInt(curSeconds % 10)) {
			allBalls(MARGIN_LEFT + 93 * (RADIUS + 1), MARGIN_TOP,
					parseInt(curHours % 10));
		}
...
function renderDigit(x, y, num, cxt) {
	...
	cxt.save();
	cxt.fillStyle = "rgb(255,255,255)";//rgb(0,102,153)
		...
		for (var j = 0; j < digit[num][i].length; j++)
			if (digit[num][i][j] == 1) {
				...
				cxt.arc(x + j * 2 * (RADIUS + 1) + (RADIUS + 1), y + i * 2
						* (RADIUS + 1) + (RADIUS + 1), RADIUS, 0, 2 * Math.PI);
				cxt.closePath();
